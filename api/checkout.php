...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Check if all required parameters are provided
    if (isset($_POST['user_id']) && isset($_POST['restaurant_id']) && isset($_POST['total_amount']) && isset($_POST['delivery_address']) && isset($_POST['menu_items']) && isset($_POST['status'])) {
        $userId = $_POST['user_id'];
        $restaurantId = $_POST['restaurant_id'];
        $totalAmount = $_POST['total_amount'];
        $deliveryAddress = $_POST['delivery_address'];
        ...
        $stmt->bind_param("iiiss", $userId, $restaurantId, $totalAmount, $deliveryAddress, $status);
